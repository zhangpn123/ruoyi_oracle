...
import com.baobiao.project.system.dict.service.IDictTypeService;
...
public class DictTypeController extends BaseController {
    ...
    @GetMapping("/getCloumn")
    @ResponseBody
    public List<DictData> getReportCloumn(@RequestParam("dictType") String dictType) {
        List<DictData> dictData = dictTypeService.selectDictDataByType(dictType);
        if (dictData != null && dictData.size() > 0) {
            for (DictData dictDatum : dictData) {
                String[] split = dictDatum.getDictValue().split("/");
                dictDatum.setDictValue(split[0]);
            }
        }
        return dictData;
    }
