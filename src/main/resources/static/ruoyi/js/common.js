...
$(function() {
	...
	// laydate 时间控件绑定只展示到月份
	if ($(".select-time-month").length > 0) {
		layui.use('laydate', function() {
			var laydate = layui.laydate;
			var startDate = laydate.render({
				elem: '#startTime',
				max: $('#endTimeMonth').val(),
				theme: 'molv',
				trigger: 'click',
				done: function(value, date) {
					// 结束时间大于开始时间
	...
	if ($(".time-input-month").length > 0) {
		layui.use('laydate', function () {
			var com = layui.laydate;
			$(".time-input-month").each(function (index, item) {
				var time = $(item);
				// 控制控件外观
