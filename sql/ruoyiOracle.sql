...
insert into sys_menu values('4', '若依官网', '0', '4', 'http://baobiao.vip', 'menuBlank', 'C', '0', '', 'fa fa-location-arrow', 'admin', TO_DATE('2018-03-16 11-33-00', 'YYYY/MM/DD HH24:MI:SS'), 'ry', TO_DATE('2018-03-16 11-33-00', 'YYYY/MM/DD HH24:MI:SS'), '若依官网地址');
-- 二级菜单
...
CREATE TABLE SYS_ASYNDOWN (
	...
	FINISHDATE VARCHAR2(50) NULL
);
CREATE UNIQUE INDEX SYS_ASYNDOWN_ID_IDX ON SYS_ASYNDOWN (ID);
COMMENT ON TABLE SYS_ASYNDOWN IS '异步下载表';
commit;

DROP TABLE SYS_CURSOR;
CREATE TABLE  SYS_CURSOR(
    ID VARCHAR2(50) NOT NULL,
    TABLENAME VARCHAR2(50) NOT NULL,
    STATUS VARCHAR2(2) NOT NULL,
    CREATEDATE VARCHAR2(50) NULL,
    UPDATEDATE VARCHAR2(50)  NULL,
);
CREATE UNIQUE INDEX_SYSCURSOR_ID_IDX ON SYS_CURSOR(ID);
COMMIT ON TABLE  SYS_CURSOR IS '临时表管理';
commit;

-- 临时统计表
CREATE TABLE SYS_TEMPTABLE(
    COCODE VARCHAR2(100)  NOT NULL,
    QUETYPE  VARCHAR2(8)  NULL,
    QUETYPENAME  VARCHAR2(100)  NULL,
    QUEDESC VARCHAR2(500) NULL,
    VOUCHERNO VARCHAR2(100)  NULL,
    REMARK VARCHAR2(500) NULL,
    RECORD VARCHAR2(100) NULL,
    ASSCHECK VARCHAR2(300) NULL,
     CREATEDATE VARCHAR2(50) NULL,
     REPORTDATE VARCHAR2(10) NULL
);
COMMENT ON COLUMN SYS_TEMPTABLE.COCODE IS '单位编码';
COMMENT ON COLUMN SYS_TEMPTABLE.QUETYPE IS '问题类型';
COMMENT ON COLUMN SYS_TEMPTABLE.QUETYPENAME IS '问题名称';
COMMENT ON COLUMN SYS_TEMPTABLE.QUEDESC IS '问题描述';
COMMENT ON COLUMN SYS_TEMPTABLE.VOUCHERNO IS '会计凭证号';
COMMENT ON COLUMN SYS_TEMPTABLE.REMARK IS '摘要';
COMMENT ON COLUMN SYS_TEMPTABLE.RECORD IS '会计分行录';
COMMENT ON COLUMN SYS_TEMPTABLE.ASSCHECK IS '辅助核算行';
COMMENT ON COLUMN SYS_TEMPTABLE.CREATEDATE IS '创建时间';
COMMENT ON COLUMN SYS_TEMPTABLE.REPORTDATE IS '统计区间';
CREATE INDEX INDEX_CO_CODE_IDX ON SYS_TEMPTABLE(COCODE);
COMMENT ON TABLE  SYS_TEMPTABLE IS '临时统计表';
commit;

-- 临时统计备份表
CREATE TABLE SYS_TEMPTABLE_HST(
   COCODE VARCHAR2(100)  NOT NULL,
    QUETYPE  VARCHAR2(8)  NULL,
    QUETYPENAME  VARCHAR2(100)  NULL,
    QUEDESC VARCHAR2(500) NULL,
...
COMMENT ON COLUMN SYS_TEMPTABLE_HST.COCODE IS '单位编码';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.QUETYPE IS '问题类型';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.QUETYPENAME IS '问题名称';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.QUEDESC IS '问题描述';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.VOUCHERNO IS '会计凭证号';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.REMARK IS '摘要';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.RECORD IS '会计分行录';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.ASSCHECK IS '辅助核算行';
COMMENT ON COLUMN SYS_TEMPTABLE_HST.CREATEDATE IS '创建时间';
COMMENT ON COLUMN SYS_TEMPTABLE.REPORTDATE IS '统计区间';
CREATE INDEX INDEX_CO_CODE_HST_IDX ON SYS_TEMPTABLE_HST(COCODE);
COMMENT ON TABLE  SYS_TEMPTABLE_HST IS '临时统计备份表';
commit;
